...
课表结构：
- 顶部表头：星期一 星期二 星期三 星期四 星期五 星期六 星期日（共7列）
...
const TEXT_PROMPT = `你是课表解析专家。以下是PDF课表的文字提取结果。

格式说明：这是一个表格，列为星期一~星期日（共7列），行为节次（1-2节、3-4节等）。
每格内容包含多行文字：课程名、节次范围、周次、校区、场地、教师等。

提取规则：
1. 遍历所有格子，每格是独立的一门课
2. 从文本中精准提取每个字段：
   - name: 第一行通常是课程名（如"高等数学AⅡ"、"大学物理B"）
   - teacher: "教师:XXX"→提取XXX（如"教师:陈明"→"陈明"）
   - location: "场地:XXX"→提取XXX（如"场地:教三204"→"教三204"）
   - weeks: 周次信息原样提取（如"(1-2节)1-13周,15-17周(单),18周"→提取"1-13周,15-17周(单),18周"）
   - dayOfWeek: 根据所在列确定（第1列=周一=1...第7列=周日=7）
   - startSection/endSection: 从节次行提取（如"(1-2节)"→startSection=1, endSection=2）
